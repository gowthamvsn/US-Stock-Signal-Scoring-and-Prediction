...
if os.path.exists(MASTER_FILE):
    ...
    print(f"📂 Existing data till {last_date.date()}")
    if last_date.date() >= expected_latest:
        print(f"✅ Already up to date. Data ends at {last_date.date()}, expected {expected_latest}")
    START_DATE = (last_date + timedelta(days=1)).strftime('%Y-%m-%d')
else:
    df_master = pd.DataFrame()
    print("🆕 No master file. Starting fresh.")
...
if not df_master.empty and df_master["Date"].max().date() >= expected_latest:
    print("📂 No need to download. Using existing master file.")
...
for ticker, group in df_analysis.groupby('Ticker'):
    ...
    # print(f"→ Future group rows: {len(future_group)}")
    if future_group.empty:
        # print("⚠️ Future group is empty")
        gain = np.nan
    else:
        current_close = group[group['Date'] == pd.to_datetime(END_DATE_ANALYSIS)]['Close']
        # print(f"→ Current close on {END_DATE_ANALYSIS}: {current_close.values}")
        if current_close.empty:
            print("⚠️ No current close price found for END_DATE_ANALYSIS")
            gain = np.nan
        else:
    ...
    # future_gains.append({'Ticker': ticker, 'FutureGain_5d': gain})
            ...
            gain = (future_max - current_close.values[0]) / current_close.values[0]
            # print(f"✅ Max future close: {future_max} on {future_max_date}, Gain: {gain:.2%}")
...
print("✅ Saved feature weights based on correlation")
...
print("✅ Saved final weighted scores")
